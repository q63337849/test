...
# -*- coding: utf-8 -*-
"""
DDPG + Spatial Attention 训练脚本（对照版）
...
from ddpg_att import DDPGAttentionAgent
...
def train(args: argparse.Namespace) -> None:
    ...
    print("=" * 60)
    print("DDPG + Spatial Attention Navigation Training (Compare)")
    print("=" * 60)
    ...
    # Action bounds (NavigationEnv does not expose gym-style action_space)
    action_low = np.array([0.0, -EnvConfig.MAX_ANGULAR_VEL], dtype=np.float32)
    action_high = np.array([EnvConfig.MAX_LINEAR_VEL, EnvConfig.MAX_ANGULAR_VEL], dtype=np.float32)
    ...
    print(f"  Sector embed dim: {args.sector_embed_dim}")
    print(f"  Attention heads: {args.spatial_att_heads}")
    print(f"  Attention dropout: {args.att_dropout}")
    print(f"\nSeed: {args.seed}")
    ...
    agent = DDPGAttentionAgent(
        ...
        n_sectors=args.n_sectors,
        sector_embed_dim=args.sector_embed_dim,
    ...
    logger = Logger(os.path.join(LOG_DIR, f"ddpg_att_training_{timestamp}.csv"))
    ...
    for episode in range(start_episode, args.num_episodes):
        ...
        for step in range(EnvConfig.MAX_STEPS):
            ...
            # --- NaN/Inf action guard (prevents env/state/reward NaN cascade) ---
            action_flat = np.asarray(action_flat, dtype=np.float32)
            if not np.all(np.isfinite(action_flat)):
                action_flat = np.nan_to_num(action_flat, nan=0.0, posinf=0.0, neginf=0.0)
            # clip to action_space bounds (keeps parity with other trainers)
            action_flat = np.clip(action_flat, action_low, action_high)
        ...
        logger.log({
            ...
            "history_len": 1,
            "att_drop_cur": float(args.att_dropout),
        ...
        if (episode + 1) > max(100, start_episode + 1) and avg_reward > best_avg_reward:
            ...
            agent.save(os.path.join(MODEL_DIR, "ddpg_att_best.pth"))
            print(f"  -> New best! Avg reward: {best_avg_reward:.2f}")

        if (episode + 1) % args.save_interval == 0:
            agent.save(os.path.join(MODEL_DIR, f"ddpg_att_ep{episode + 1}.pth"))
    ...
    agent.save(os.path.join(MODEL_DIR, "ddpg_att_final.pth"))

    plot_training_curves(
        episode_rewards,
        episode_steps,
        save_path=os.path.join(RESULT_DIR, f"ddpg_att_training_curves_{timestamp}.png"),
    )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="DDPG + Spatial Attention Navigation Training (Compare)")
    ...
    # 空间注意力参数（agent 原样）
    p.add_argument("--sector_embed_dim", type=int, default=32)
