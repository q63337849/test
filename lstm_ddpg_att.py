...
class SpatialSectorAttention(nn.Module):
    """Beam/Sector-level 空间注意力（真·多头 Cross-Attention）：
    - Query: non-lidar(+aux) 每个时间步一个 query
    - Key/Value: N 个 sectors tokens
    - 支持距离偏置/有效性偏置（不做 hard mask，避免空旷场景全无回波导致数值问题）
    """
    ...
    def __init__(
        ...
        self.model_dim = int(sector_model_dim)
        self.num_heads = int(num_heads)
        if self.model_dim % self.num_heads != 0:
            raise ValueError(f"sector_model_dim({self.model_dim}) 必须能被 num_heads({self.num_heads}) 整除")
        self.head_dim = self.model_dim // self.num_heads

        # token features: [range_norm, (optional) diff, valid_flag, sin(theta), cos(theta)]
        token_in = 2 + (1 if layout.has_lidar_diff else 0) + 2  # range, valid, (+diff), +pos(sin,cos)

        # sector positional encoding (cyclic): sin/cos(2π·i/N)
        theta = torch.arange(self.n, dtype=torch.float32) / float(self.n) * (2.0 * math.pi)
        pos = torch.stack([torch.sin(theta), torch.cos(theta)], dim=-1)  # (N,2)
        self.register_buffer("pos_enc", pos)

        # true multi-head cross-attention projections
        self.q_proj = nn.Linear(non_lidar_dim, self.model_dim)
        self.k_proj = nn.Linear(token_in, self.model_dim)
        self.v_proj = nn.Linear(token_in, self.model_dim)
        self.out_proj = nn.Linear(self.model_dim, self.model_dim)

        self.att_drop = nn.Dropout(float(att_dropout))

        # learnable biases (init: closer is more important; invalid slightly penalized)
    ...
    def forward(
            ...
            lidar_diff: Optional[torch.Tensor],
        ...
        pos = self.pos_enc.view(1, 1, N, 2).expand(B, H, N, 2)
        feats.append(pos)
        tok = torch.cat(feats, dim=-1)  # (B,H,N,token_in)

        # projections
        q = self.q_proj(non_lidar).unsqueeze(2)  # (B,H,1,Dm)
        k = self.k_proj(tok)                     # (B,H,N,Dm)
        v = self.v_proj(tok)                     # (B,H,N,Dm)

        # reshape to heads
        BH = B * H
        q = q.reshape(BH, 1, self.model_dim)
        k = k.reshape(BH, N, self.model_dim)
        v = v.reshape(BH, N, self.model_dim)

        q = q.view(BH, 1, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # (BH,heads,1,dh)
        k = k.view(BH, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # (BH,heads,N,dh)
        v = v.view(BH, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # (BH,heads,N,dh)

        score = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)  # (BH,heads,1,N)

        # additive biases (broadcast to heads)
        if self.use_distance_bias:
            dist_bias = (1.0 - sectors).reshape(BH, 1, 1, N)
            score = score + self.range_bias_scale * dist_bias

        if self.use_valid_bias:
            vb = (self.valid_bias * valid + self.invalid_bias * (1.0 - valid)).reshape(BH, 1, 1, N)
            score = score + vb

        attn = torch.softmax(score, dim=-1)  # (BH,heads,1,N)
        attn = self.att_drop(attn)

        ctx = torch.matmul(attn, v)  # (BH,heads,1,dh)
        ctx = ctx.permute(0, 2, 1, 3).contiguous().view(BH, 1, self.model_dim).squeeze(1)  # (BH,Dm)

        ctx = self.out_proj(ctx)
        ctx = self.out_ln(ctx)

        return ctx.view(B, H, self.model_dim)



class SpatialSectorAttentionV2(nn.Module):
    """
    V2++：在你当前 V2 的基础上增加
    - hard mask：无效 token 不参与 softmax（避免均匀化）
    - pre-norm + residual：训练更稳
    - ffn：提高表达能力（轻量 Transformer block）
    - 条件化残差：把 non_lidar 显式注入 ctx（更“任务条件化”）
    """
    ...
    def __init__(
        ...
        no_echo_threshold: float = 0.95,
        smooth_valid_tau: float = 0.03,
        use_pos: bool = True,
        **_ignored,
    ):
        super().__init__()

        # 兼容：att_dropout -> attn_dropout
        if att_dropout is not None:
            attn_dropout = float(att_dropout)

        # 兼容：num_heads -> n_queries（这里 num_heads 表示并行 query 数）
        if n_queries is None:
            n_queries = int(num_heads)

        self.layout = layout
        self.n = int(layout.n_sectors)
        self.no_echo_threshold = float(no_echo_threshold)
        self.tau = float(smooth_valid_tau)
        self.nq = int(n_queries)
        self.use_pos = bool(use_pos)

        # token dims: range + valid_conf + (diff,closing,inv_ttc) + pos(sin,cos)
        base = 2
        dyn = 3 if layout.has_lidar_diff else 0
        pos = 2 if self.use_pos else 0
        token_in = base + dyn + pos

        self.token_proj = nn.Sequential(
            nn.Linear(token_in, sector_model_dim),
            nn.ReLU(),
            nn.Linear(sector_model_dim, sector_model_dim),
        )

        # multi-query: non_lidar -> (Q * Dm)
        self.query_proj = nn.Linear(non_lidar_dim, self.nq * sector_model_dim)
        self.query_prompt = nn.Parameter(torch.zeros(self.nq, sector_model_dim))

        # fuse Q contexts back to one
        self.reduce = nn.Linear(self.nq * sector_model_dim, sector_model_dim)

        # additive biases（soft bias，和 hard mask 互补）
        self.range_bias_scale = nn.Parameter(torch.tensor(1.0))
        self.valid_bias = nn.Parameter(torch.tensor(0.0))
        self.invalid_bias = nn.Parameter(torch.tensor(-1.0))
        ...
        # === 新增：Pre-Norm / Residual / FFN ===
        self.tok_ln = nn.LayerNorm(sector_model_dim)
        self.q_ln = nn.LayerNorm(sector_model_dim)
        self.ctx_ln = nn.LayerNorm(sector_model_dim)

        # non_lidar -> ctx residual（任务条件化残差）
        self.ctx_cond_proj = nn.Linear(non_lidar_dim, sector_model_dim)

        ffn_hidden = max(64, 2 * sector_model_dim)
        self.ffn = nn.Sequential(
            nn.Linear(sector_model_dim, ffn_hidden),
            nn.ReLU(),
            nn.Dropout(float(attn_dropout)),
            nn.Linear(ffn_hidden, sector_model_dim),
        )
        self.ffn_drop = nn.Dropout(float(attn_dropout))

        if self.use_pos:
            theta = torch.arange(self.n, dtype=torch.float32) / float(self.n) * (2.0 * math.pi)
            pos_enc = torch.stack([torch.sin(theta), torch.cos(theta)], dim=-1)  # (N,2)
    ...
    def forward(
        ...
        """
        sectors: (B,H,N) range_norm in [0,1]
        lidar_diff: (B,H,N) or None
        non_lidar: (B,H,D_non_aug)
        return: (B,H,Dm)
        """
        B, H, N = sectors.shape
        assert N == self.n

        # smooth valid confidence（用于 soft bias）
        valid_conf = torch.sigmoid((self.no_echo_threshold - sectors) / (self.tau + 1e-6))

        feats = [sectors.unsqueeze(-1), valid_conf.unsqueeze(-1)]

        if lidar_diff is not None:
            diff = torch.clamp(lidar_diff, -1.0, 1.0)
            closing = F.relu(-diff)
            inv_ttc = closing / (sectors + 1e-3)
            inv_ttc = torch.clamp(inv_ttc, 0.0, 10.0)
            feats += [diff.unsqueeze(-1), closing.unsqueeze(-1), inv_ttc.unsqueeze(-1)]
        ...
        tok = torch.cat(feats, dim=-1)          # (B,H,N,token_in)
        tok_emb = self.token_proj(tok)          # (B,H,N,Dm)
        tok_emb = self.tok_ln(tok_emb)          # Pre-Norm token

        # multi-query（Pre-Norm query）
        q = self.query_proj(non_lidar).view(B, H, self.nq, -1)   # (B,H,Q,Dm)
        q = q + self.query_prompt.view(1, 1, self.nq, -1)
        q = self.q_ln(q)

        # score: (B,H,Q,N)
        score = torch.einsum("bhqd,bhnd->bhqn", q, tok_emb) / math.sqrt(tok_emb.shape[-1])

        # soft biases
        score = score + self.range_bias_scale * (1.0 - sectors).unsqueeze(2)
        score = score + self.valid_bias * valid_conf.unsqueeze(2) + self.invalid_bias * (1.0 - valid_conf).unsqueeze(2)

        # === hard mask：无效 token 不进入 softmax ===
        # hard_valid: True 表示有效，False 表示无效(no-echo)
        hard_valid = (sectors < self.no_echo_threshold)  # (B,H,N) bool
        # 若某个时刻全部无效，则放开 mask（避免 softmax 全 -inf 导致 NaN）
        all_invalid = (~hard_valid).all(dim=-1, keepdim=True)  # (B,H,1)
        hard_valid = hard_valid | all_invalid                 # (B,H,N)
        score = score.masked_fill(~hard_valid.unsqueeze(2), -1e9)

        # temperature + softmax
        temp = torch.exp(self.log_temp).clamp(0.3, 2.0)
        w = torch.softmax(score / temp, dim=-1)  # (B,H,Q,N)
        w = self.attn_drop(w)

        # ctx per query: (B,H,Q,Dm)
        ctx_q = torch.einsum("bhqn,bhnd->bhqd", w, tok_emb)

        # fuse queries -> ctx: (B,H,Dm)
        ctx = self.reduce(ctx_q.reshape(B, H, self.nq * tok_emb.shape[-1]))

        # === 条件化残差 + LN ===
        ctx = ctx + self.ctx_cond_proj(non_lidar)
        ctx = self.ctx_ln(ctx)

        # === FFN residual（轻量 Transformer block）===
        ctx = ctx + self.ffn_drop(self.ffn(ctx))
        ctx = self.ctx_ln(ctx)
...
class TemporalAttentionPool(nn.Module):
    """时间注意力池化（真·多头 Cross-Attention）：
    - Query: last_hidden（最后一个时间步的 LSTM 隐状态）
    - Key/Value: LSTM 输出序列 out[:,t,:]
    - 额外加性偏置：recency、diff_mag、dyaw_abs（与原版保持一致）
    """
    ...
    def __init__(
        ...
        if self.att_dim % self.num_heads != 0:
            raise ValueError(f"temporal_att_dim({self.att_dim}) 必须能被 temporal_att_heads({self.num_heads}) 整除")
        self.head_dim = self.att_dim // self.num_heads

        self.q_proj = nn.Linear(self.hidden_dim, self.att_dim)
        self.k_proj = nn.Linear(self.hidden_dim, self.att_dim)
        self.v_proj = nn.Linear(self.hidden_dim, self.att_dim)
        self.out_proj = nn.Linear(self.att_dim, self.hidden_dim)

        self.att_drop = nn.Dropout(float(att_dropout))
        self.out_drop = nn.Dropout(float(att_dropout))

        # time-feature bias (learnable scalars)
    ...
    def forward(
        ...
        q = self.q_proj(last_hidden).unsqueeze(1)  # (B,1,att_dim)
        k = self.k_proj(lstm_out)                  # (B,H,att_dim)
        v = self.v_proj(lstm_out)                  # (B,H,att_dim)

        # reshape to heads
        q = q.view(B, 1, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # (B,heads,1,dh)
        k = k.view(B, H, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # (B,heads,H,dh)
        v = v.view(B, H, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # (B,heads,H,dh)

        score = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)  # (B,heads,1,H)

        # recency bias: position in [0,1], later larger
        pos = torch.linspace(0.0, 1.0, H, device=lstm_out.device, dtype=lstm_out.dtype).view(1, 1, 1, H)
        score = score + self.recency_scale * pos

        if self.use_time_features and time_feats is not None and time_feats.shape[-1] >= 2:
            diff_mag = time_feats[:, :, 0].view(B, 1, 1, H)
            dyaw_abs = time_feats[:, :, 1].view(B, 1, 1, H)
            score = score + self.diff_w * diff_mag + self.yaw_w * (1.0 - dyaw_abs)

        attn = torch.softmax(score, dim=-1)  # (B,heads,1,H)
        attn = self.att_drop(attn)

        ctx = torch.matmul(attn, v)  # (B,heads,1,dh)
        ctx = ctx.permute(0, 2, 1, 3).contiguous().view(B, 1, self.att_dim).squeeze(1)  # (B,att_dim)
...
class AttStateEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_att = SpatialSectorAttentionV2(
            layout=self.layout,
            non_lidar_dim=self.non_lidar_aug_dim,
            sector_model_dim=sector_model_dim,
            num_heads=int(spatial_att_heads),
        ...
        # raw ordered sectors residual (keep direction): [sectors, (optional) lidar_diff]
        raw_dim = int(self.layout.n_sectors) + (int(self.layout.n_sectors) if self.layout.has_lidar_diff else 0)
        self.raw_proj = nn.Linear(raw_dim, half)

        self.fuse = nn.Linear(half * 3, self.embed_dim)
    ...
    def forward(self, state_seq: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # raw ordered sectors residual embedding (preserve direction)
        if lidar_diff is not None:
            raw_vec = torch.cat([sectors, lidar_diff], dim=-1)  # (B,H,2N)
        else:
            raw_vec = sectors  # (B,H,N)
        raw_emb = F.relu(self.raw_proj(raw_vec))  # (B,H,half)
        raw_emb = self.dropout(raw_emb)

        x = torch.cat([non_emb, sp_emb, raw_emb], dim=-1)  # (B,H,3*half)
...
class LSTMActorAtt(nn.Module):
    ...
    def __init__(
        ...
        self.temporal_att = TemporalAttentionPool(
            ...
            num_heads=int(temporal_att_heads),
