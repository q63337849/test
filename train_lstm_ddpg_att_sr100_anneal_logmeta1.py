...
from lstm_ddpg_att1 import LSTMDdpgAgent
...
def set_global_seeds(seed: int) -> None:
    """Best-effort seeding for Python/NumPy/PyTorch."""
    try:
        import random
        random.seed(seed)
    except Exception:
...
def seed_env(env, seed: int) -> None:
    """Best-effort seeding for gym-like env/action_space."""
    # Gymnasium style
    try:
        env.reset(seed=seed)
        return
...
def train(args: argparse.Namespace) -> None:
    ...
    print(f"  sp_gate_init: {args.sp_gate_init} (sigmoid={1/(1+math.exp(-args.sp_gate_init)):.3f})")
    print(f"  temporal_gate_init: {args.temporal_gate_init} (sigmoid={1/(1+math.exp(-args.temporal_gate_init)):.3f})")
    ...
    agent = LSTMDdpgAgent(
        ...
        sp_gate_init=args.sp_gate_init,
    ...
    logger = Logger(os.path.join(LOG_DIR, f"lstm_ddpg_att1_training_{timestamp}.csv"))
    ...
    for episode in range(start_episode, args.num_episodes):
        ...
        # e.g., args.att_dropout=0.05: 0.05 (0~40%) -> 0.00 (40~100%)
        att_drop_cur = _lin_anneal(progress, args.att_dropout, 0.0, 0.40, 1.00)

        # Temperature anneal: start softer, end sharper (early stability, late precision)
        # 1.15 -> 0.80 over 20%~100%
        att_temp_cur = _lin_anneal(progress, 1.15, 0.80, 0.20, 1.00)

        # Tau anneal: start smoother valid_conf, end slightly sharper
        # 0.05 -> 0.03 over 0%~80%
        att_tau_cur = _lin_anneal(progress, 0.05, 0.03, 0.00, 0.80)
        ...
        if avg_reward > best_reward and (episode + 1) > 100:
            ...
            agent.save(os.path.join(MODEL_DIR, "lstm_ddpg_att1_best.pth"))
            print(f"  -> New best! Avg reward: {best_reward:.2f}")

        if (episode + 1) % args.save_interval == 0:
            agent.save(os.path.join(MODEL_DIR, f"lstm_ddpg_att1_ep{episode + 1}.pth"))
    ...
    agent.save(os.path.join(MODEL_DIR, "lstm_ddpg_att1_final.pth"))

    plot_training_curves(
        episode_rewards,
        episode_steps,
        save_path=os.path.join(RESULT_DIR, f"lstm_ddpg_att1_training_curves_{timestamp}.png"),
    )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--seed", type=int, default=0, help="Random seed for reproducibility")  # 训练速度相关
    ...
    p.add_argument("--sp_gate_init", type=float, default=0.0,
                    help="Spatial gate init. 0.0=50/50 blend, -2.0=suppress att, -4.0=almost off")
    p.add_argument("--temporal_gate_init", type=float, default=-2.0,
                    help="Temporal gate init. 0.0=50/50, -2.0=suppress temporal att")
