...
class SpatialAttentionV3(nn.Module):
    """真正的多头空间注意力 V3 — 适配序列输入 (B,H,N).

    从 ddpg_att V3 移植，关键改进（vs 旧版 V2）：
      - V2: heads 被忽略, 实际只是 multi-query softmax → 表达力有限
      - V3: 真正的 H 个注意力头, 每个头独立聚焦不同空间模式
    ...
    稳定性机制:
      ...
      - 硬掩码: 无回波扇区从 softmax 排除 (all-invalid → 均匀回退)
      - 可学习温度: per-head, clamped 0.3-2.0
      - 残差: attention + mean_pooling
      - Post FFN + LayerNorm
    """
    TOKEN_DIM = 6
    ...
    def __init__(
        ...
        **_ignored,
        ...
        assert self.model_dim % self.heads == 0, (
            f"sector_model_dim({self.model_dim}) must be divisible by num_heads({self.heads})")
        self.no_echo_threshold = float(no_echo_threshold)
        self.has_diff = bool(layout.has_lidar_diff)

        # Token projection: 6D → model_dim
        self.token_proj = nn.Sequential(
            nn.Linear(self.TOKEN_DIM, self.model_dim),
            nn.ReLU(),
            nn.Linear(self.model_dim, self.model_dim),
        )
        self.token_ln = nn.LayerNorm(self.model_dim)

        # Query projection: non_lidar → model_dim
        self.query_proj = nn.Sequential(
            nn.Linear(int(non_lidar_dim), self.model_dim),
            nn.ReLU(),
            nn.Linear(self.model_dim, self.model_dim),
        )
        self.query_ln = nn.LayerNorm(self.model_dim)

        # Learnable temperature (one per head)
        self.log_temp = nn.Parameter(torch.zeros(self.heads))

        # Output projection
        self.out_proj = nn.Linear(self.model_dim, self.model_dim)

        # Mean-pool residual
        self.mean_proj = nn.Linear(self.model_dim, self.model_dim)

        # FFN + LayerNorm
        ffn_hidden = max(64, 2 * self.model_dim)
        self.ffn = nn.Sequential(
            nn.Linear(self.model_dim, ffn_hidden),
            nn.ReLU(),
            nn.Linear(ffn_hidden, self.model_dim),
        )
        self.out_ln = nn.LayerNorm(self.model_dim)
        self.dropout = nn.Dropout(float(att_dropout))

        # Fixed positional encoding
        angles = torch.arange(self.n, dtype=torch.float32) * (2.0 * math.pi / self.n)
        self.register_buffer('pos_sin', torch.sin(angles))
        self.register_buffer('pos_cos', torch.cos(angles))
    ...
    def forward(
        ...
        # Flatten (B,H,...) → (BH,...) for batch processing
        sec_flat = sectors.reshape(B * H, N)
        diff_flat = lidar_diff.reshape(B * H, N) if lidar_diff is not None else None
        non_flat = non_lidar.reshape(B * H, non_lidar.shape[-1])

        out_flat = self._forward_flat(sec_flat, diff_flat, non_flat)  # (BH, model_dim)
        return out_flat.view(B, H, self.model_dim)
    ...
    def _forward_flat(
        ...
        """Process flattened batch: sectors (BH, N), returns (BH, model_dim)."""
        BH, N = sectors.shape
        Hd = self.heads
        d_h = self.head_dim
        ...
        closing = torch.clamp(diff, -1.0, 0.0)

        pos_sin = self.pos_sin.unsqueeze(0).expand(BH, -1)
        pos_cos = self.pos_cos.unsqueeze(0).expand(BH, -1)

        tokens_raw = torch.stack([sectors, valid, diff, closing, pos_sin, pos_cos], dim=-1)  # (BH,N,6)

        # === Project ===
        tokens = self.token_ln(self.token_proj(tokens_raw))   # (BH,N,D)
        query = self.query_ln(self.query_proj(non_lidar))      # (BH,D)

        # === Multi-head attention ===
        t_mh = tokens.view(BH, N, Hd, d_h).transpose(1, 2)   # (BH,Hd,N,d_h)
        q_mh = query.view(BH, Hd, d_h).unsqueeze(2)           # (BH,Hd,1,d_h)

        temp = torch.clamp(self.log_temp.exp(), 0.3, 2.0)
        scale = (d_h ** 0.5) * temp.view(1, Hd, 1)
        scores = (q_mh * t_mh).sum(dim=-1) / scale            # (BH,Hd,N)

        # === Hard mask ===
        hard_mask = (sectors < self.no_echo_threshold)         # (BH,N)
        hard_mask_mh = hard_mask.unsqueeze(1).expand(-1, Hd, -1)
        any_valid = hard_mask_mh.any(dim=-1, keepdim=True)

        scores = scores.masked_fill(~hard_mask_mh, -1e9)
        weights = torch.softmax(scores, dim=-1)

        uniform = torch.ones(BH, Hd, N, device=sectors.device) / N
        weights = torch.where(any_valid.expand_as(weights), weights, uniform)
        weights = self.dropout(weights)

        # Weighted sum → concat heads
        attended = torch.einsum('bhn,bhnd->bhd', weights, t_mh)
        attended = attended.reshape(BH, self.model_dim)
